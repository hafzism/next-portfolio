...
const SideNav = () => {
  ...
  return (
    <>
      ...
      <nav className={cn(
        "fixed left-4 lg:left-6 top-1/2 -translate-y-1/2 z-50 hidden lg:flex flex-col gap-2 lg:gap-4 p-2 lg:p-3 rounded-2xl bg-white/5 backdrop-blur-xl border shadow-2xl transition-colors duration-300",
          ...
          return (
            <div key={item.path} className="relative group">
              <Link href={item.path}>
                <motion.div
                  className={cn(
                    "w-10 h-10 lg:w-12 lg:h-12 rounded-xl flex items-center justify-center relative z-20 transition-colors duration-300",
      ...
      <nav className={cn(
        "fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex lg:hidden items-center gap-1 p-2 rounded-2xl bg-white/10 backdrop-blur-lg border shadow-2xl transition-colors duration-300",
        isDark ? "border-white/10" : "border-black"
      )}>
        {navItems.map((item) => {
          const Icon = item.icon;
